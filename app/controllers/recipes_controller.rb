class RecipesController < ApplicationController
  ...
  def index
    @recipes = Recipe.includes(%i[user recipe_foods]).where(user_id: current_user.id).order('created_at DESC')
  ...
  def public
    @public_recipes = Recipe.includes(%i[user recipe_foods]).where(public: true).order('created_at DESC')
